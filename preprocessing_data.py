...
def extract_text(path):
    ...
    ID, name, time, respondent, interviewer = get_meta_data(doc)
    ...
    full_text = extract(full_text, speaker_label='Z')
    full_text = extract(full_text, speaker_label='Y2')
    full_text = extract(full_text, speaker_label='Y3')
    full_text = extract(full_text, speaker_label=interviewer)
...
def extract(text, speaker_label):
    ...
    def cleaning(match):
        ...
        return '\n'.join(cleaned) + '\n' if cleaned else ''
    return re.sub(pattern, cleaning, text, flags=re.DOTALL)
